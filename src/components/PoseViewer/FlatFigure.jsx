...
const S  = 90
const OX = VW / 2
const OY = VH - 28     // world y=0 → SVG y=272
    ...
    >
      <defs>
        <radialGradient id="pose-glow" cx="50%" cy="55%" r="50%">
          <stop offset="0%"   stopColor="#9C6FFF" stopOpacity="0.2" />
      ...
      <rect x="20" y={footY + 6} width={VW - 40} height="16" rx="8"
            fill={MAT} opacity="0.85" />
      <line x1="28" y1={footY + 14} x2={VW - 28} y2={footY + 14}
            stroke="#FFB3B3" strokeWidth="1" strokeDasharray="6 4" opacity="0.6" />

      {/* Ground shadow */}
      <ellipse cx={OX} cy={footY + 10} rx="30" ry="5" fill="#000" opacity="0.15" />
      ...
      <circle cx={bunX - 2} cy={bunY - 2} r={3} fill="#5C3D2A" opacity="0.4" />

      {/* Head */}
      <circle cx={hx} cy={hy} r={18} fill={SKIN} />

      {/* Blush */}
      <circle cx={hx - 8} cy={hy + 4} r={5} fill="#FF8A80" opacity="0.22" />
      <circle cx={hx + 8} cy={hy + 4} r={5} fill="#FF8A80" opacity="0.22" />

      {/* Face — rotates with head through all poses */}
      <g transform={`rotate(${faceAngle},${hx},${hy})`}>
        ...
        <path d={`M${hx+2} ${hy-2} Q${hx+5.5} ${hy-7} ${hx+9} ${hy-2}`}
              stroke={HAIR} strokeWidth="2" fill="none" strokeLinecap="round" />
        <circle cx={hx} cy={hy+4} r={1.5} fill="#C4705A" opacity="0.55" />
